...
frappe.ui.form.on('Client', {
	refresh: function(frm) {
		...
		frappe.call({
			"method": "charity.charity_management_system.tools.get_entitlements",
			async: false,
			args: {
			  doc_name: frm.doc.name,
			},
			callback: function(r) {				
				$("#entitlements").html(frappe.render_template("customer_entitlements",{"coupon":r.message.coupon,"cheque_specification":r.message.cheque_specification}));
			}
		  });
	},
...
frappe.ui.form.on("Winter Cladding Table", {
    date_of_receive: function(frm, cdt, cdn) {
		debugger;
		var row = locals[cdt][cdn];

		frappe.model.set_value(row.doctype, row.name, "next_receive_date", frappe.datetime.add_months(row.date_of_receivee, 12));
		frm.refresh_field("winter_cladding_table");

	}, 
	date_of_receiving_heaters: function(frm, cdt, cdn) {
		debugger;
		var row = locals[cdt][cdn];

		frappe.model.set_value(row.doctype, row.name, "next_receive_heaters_date", frappe.datetime.add_months(row.date_of_receiving_heaters, 36));
		frm.refresh_field("winter_cladding_table");

	},
	date_of_receiving_blanket: function(frm, cdt, cdn) {
		debugger;
		var row = locals[cdt][cdn];

		frappe.model.set_value(row.doctype, row.name, "next_receiving_blanket", frappe.datetime.add_months(row.date_of_receiving_blanket, 36));
		frm.refresh_field("winter_cladding_table");
